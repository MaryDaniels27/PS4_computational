...
mutable struct Results_NSS
    ...
    θ::Float64 #labor income tax, when there's no social security θ = 0 and with social security θ = 0.11
end

function Initialize_NSS()
    prim = Primitives() #initialize primtiives
    val_func = zeros(prim.na, prim.nz, prim.N) #initial value function guess
    val_func[:, :, :] .= -50   #with no SS
    ...
    K_0 = 4.481   #aggregate capital without SS
    L_0 = 0.3634   #aggregate labor without SS 
    w = 1.581
    r = 0.012
    b = 0
    θ = 0.0

    res_NSS = Results_NSS(val_func, pol_func, labor, r, w, b, K_0, L_0, θ) #initialize results struct
    prim, res_NSS #return deliverables
end

#Initializing the population distribution 

mutable struct mu_results_NSS
    mu::Array{Float64, 1}
    mu_dist::Array{Float64, 3}
    mu1_NSS::Array{Float64, 3}
end

function Initialize_3_NSS()
    @unpack N, n, na, nz = prim
    mu_dist = zeros(prim.na, prim.nz, prim.N) #this will hold our unweighted distribution
    mu1_NSS = zeros(prim.na, prim.nz, prim.N)  #this will hol our final, properly weight distribution
    mu = ones(prim.N)
    for i = 2:N
    ...
    mass_NSS = mu_results_NSS(mu, mu_dist, mu1_NSS)
end
...
function Bellman(prim::Primitives, res_NSS::Results_NSS)
    @unpack N, R, tw, γ, σ, η, z, markov, β, a_grid, na, nz = prim
    @unpack val_func, r, w, b, θ = res_NSS
    ...
    for a_index = 1:na, z_index = 1:nz #looping over assets and states
        ...
        # res_NSS.val_func[a_index, z_index, 66] = val_N #storing last period utility in the value function
        # res_NSS.pol_func[a_index, z_index, 66] = 0.0 #storing the last period policy function
        #In the No S.S. case we add an if esle condition
        if val_N > -50
            res_NSS.val_func[a_index, z_index, 66] = val_N #storing last period utility in the value function
            res_NSS.pol_func[a_index, z_index, 66] = 0.0 #storing the last period policy function
        elseif val_N < -50
            res_NSS.val_func[a_index, z_index, 66] = 0.0 #storing last period utility in the value function
            res_NSS.pol_func[a_index, z_index, 66] = 0.0 #storing the last period policy function
        end
    end
    for j = (N-1):-1:R
        for a_index = 1:na, z_index = 1:nz  #looping over assets today
            ...
            for ap_index = 1:na #looping over assets tomorrow
                ...
                if c>0  #check for positivity
                    val = (c^((1-σ)*γ))/(1-σ) + β * res_NSS.val_func[ap_index, z_index, j+1] #calculate the value function while looking at next period's value function
                    if val > candidate_max
                        ...
                        res_NSS.pol_func[a_index, z_index, j] = a_grid[ap_index] #update the policy function
                        res_NSS.val_func[a_index, z_index, j] = val  #updating the value function
                    end
                end
            end
        end
        ...
        for a_index = 1:na  #looping over assets today
            ...
            for z_index = 1:nz #looping for productivity states
                ...
                for ap_index = 1:na #looping over assets tomorrow
                    ...
                    c = budget - a_grid[ap_index] #consumption given a' selection
                        if c>0  #check for positivity
                            val = (((c^γ)*((1-l)^(1-γ)))^(1-σ))/(1-σ) + β * sum(res_NSS.val_func[ap_index, :, i+1].* markov[z_index, :]) #calculate the value 
                            #function while looking at next period's value function
                                if val > candidate_max
                                    candidate_max = val  #update candidate max
                                    res_NSS.pol_func[a_index, z_index, i] = a_grid[ap_index] #update the policy function
                                    res_NSS.val_func[a_index, z_index, i] = val  #updating the value function
                                    res_NSS.labor[a_index, z_index, i] = l  #updating the optimal labor supply vector 
                                end
                        end
                end
            end
...
function Distribution(prim::Primitives, res_NSS::Results_NSS, mass_NSS::mu_results_NSS)
    @unpack N, R, tw, z, a_grid, na, nz, markov = prim
    @unpack pol_func = res
    @unpack mu, mu_dist, mu1_NSS = mass_NSS
    ...
    for i = 1:prim.N
        mu1_NSS[:, :, i] = mu_dist[:, :, i] .* mu[i]
    end
    mass_NSS.mu_dist = mu_dist  #updating the unweighted distribution
    mass_NSS.mu1_NSS = mu1_NSS  #updating the weighted distribution 
end

function Solve_model(prim::Primitives, res_NSS::Results_NSS, mass_NSS::mu_results_NSS)
    Bellman(prim, res_NSS) #solve the Bellman!
    Distribution(prim, res_NSS, mass_NSS)   #solve for the stationary distribution
end
