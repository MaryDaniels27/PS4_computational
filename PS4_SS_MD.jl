...
function Initialize()
    ...
    θ = 0.11 #labor income tax, when there's no social security θ = 0 and with social security θ = 0.11

    ### Without social security  ####
    # K_0 = 4.482   #aggregate capital without SS
    ...
    res = Results(val_func, pol_func, labor, r, w, b, K_0, L_0, θ) #initialize results struct
...
function Bellman(prim::Primitives, res::Results)
    ...
    for a_index = 1:na, z_index = 1:nz #looping over assets and states
        ...
        #     res.pol_func[a_index, z_index, 66] = 0.0 #storing the last period policy function
        # elseif val_N < -50
        #     res.val_func[a_index, z_index, 66] = 0.0 #storing last period utility in the value function
        #     res.pol_func[a_index, z_index, 66] = 0.0 #storing the last period policy function
        # end
    end
    for j = (N-1):-1:R
